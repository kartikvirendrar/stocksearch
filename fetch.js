    ...
    document.getElementById('searchBar').addEventListener('keyup', (e) => {
        ...
        async function fetchapi() {
            if(a==stock1.length){
            await fetch(`https://stock-data-yahoo-finance-alternative.p.rapidapi.com/v6/finance/quote?symbols=${e.target.value}`, {
                        "method": "GET","headers": {"x-rapidapi-host": "stock-data-yahoo-finance-alternative.p.rapidapi.com",
            ...
            const filterStocks = stock1.filter( stock => {
                if(stock.quoteType == "EQUITY"){
                    return (stock.longName.toLowerCase().includes(e.target.value.toLowerCase()) ||
    ...
    fetch("https://stock-data-yahoo-finance-alternative.p.rapidapi.com/ws/screeners/v1/finance/screener/predefined/saved?scrIds=day_gainers&count=250", {
	"method": "GET","headers": {"x-rapidapi-host": "stock-data-yahoo-finance-alternative.p.rapidapi.com",
		"x-rapidapi-key": "a5d0d6aeebmshb035568326931a4p160069jsn76fdafc068cd"
	}})
    .then(response => {
    ...
    const displayStocks = (stocks) => {
        let x =-1;
        stockls=stocks;
        const htmlString = stocks.map((stocks) => {
            x+=1;
            return `
            ...
            document.getElementById('details').style.color = 'white'; ">
                <div class="box1">
            	<h1 style="color:${stocks.twoHundredDayAverageChangePercent.toFixed(2)>=0?'green':'red'};" >${stocks.symbol}</h1></div>         
                <div class="box2">   
				<h2>${stocks.longName}</h2><hr>
                <h3>Price : ${stocks.regularMarketPrice} ${stocks.currency} | Range : ${stocks.regularMarketDayRange}</h3>
                <h3>Regular Market Change : ${stocks.regularMarketChange.toFixed(2)} (${stocks.regularMarketChangePercent.toFixed(2)}%)</h3>
                <h3>Average Change Percentage : ${stocks.twoHundredDayAverageChangePercent.toFixed(2)}%</h3></div>
            </div><br><br>`;
        }).join('');
